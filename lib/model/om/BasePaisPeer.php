...
abstract class BasePaisPeer {
	...
	/**
	 * An identiy map to hold any loaded instances of Pais objects.
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		$criteria->setPrimaryTableName(PaisPeer::TABLE_NAME);
		...
		{
		  call_user_func($sf_hook, 'BasePaisPeer', $criteria, $con);
		}
	...
	{
		return PaisPeer::populateObjects(PaisPeer::doSelectStmt($criteria, $con));
	}
	/**
	 ...
	 *
	 * @param      Pais $value A Pais object.
	 ...
	 *
	 * @param      mixed $value A Pais object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof Pais) {
				$key = (string) $value->getId();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or Pais object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     Pais Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
	...
	/**
	 * Method to invalidate the instance pool of all tables related to pais
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key = PaisPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj = PaisPeer::getInstanceFromPool($key))) {
				...
				$obj->hydrate($row);
				$results[] = $obj;
				PaisPeer::addInstanceToPool($obj, $key);
			} // if key exists
		}
		$stmt->closeCursor();
		return $results;
	...
	{
		$key = PaisPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = PaisPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + PaisPeer::NUM_COLUMNS;
		} else {
			$cls = PaisPeer::OM_CLASS;
			$obj = new $cls();
	...
	{
	  $dbMap = Propel::getDatabaseMap(BasePaisPeer::DATABASE_NAME);
	  if (!$dbMap->hasTable(BasePaisPeer::TABLE_NAME))
	  {
	...
	{
		return $withPrefix ? PaisPeer::CLASS_DEFAULT : PaisPeer::OM_CLASS;
	}
	...
	public static function doInsert($values, PropelPDO $con = null)
	{
		...
		if ($criteria->containsKey(PaisPeer::ID) && $criteria->keyContainsValue(PaisPeer::ID) ) {
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(PaisPeer::ID);
			if ($value) {
				$selectCriteria->add(PaisPeer::ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(PaisPeer::TABLE_NAME);
			}
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(PaisPeer::TABLE_NAME, $con, PaisPeer::DATABASE_NAME);
			// Because this db requires some delete cascade/set null emulation, we have to
			// clear the cached instance *after* the emulation has happened (since
			// instances get re-added by the select statement contained therein).
			PaisPeer::clearInstancePool();
			PaisPeer::clearRelatedInstancePool();
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			...
			PaisPeer::removeInstanceFromPool($values);
			...
			foreach ((array) $values as $singleval) {
				PaisPeer::removeInstanceFromPool($singleval);
			}
		}
	...
	public static function doValidate(Pais $obj, $cols = null)
	{
		...
		return BasePeer::doValidate(PaisPeer::DATABASE_NAME, PaisPeer::TABLE_NAME, $columns);
	}
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = PaisPeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BasePaisPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
